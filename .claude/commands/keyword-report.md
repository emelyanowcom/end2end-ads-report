...
Прочитай `.env`. Если файл существует и уже содержит переменные — переходи к Шагу 2.

Если `.env` пустой или не существует — это первый запуск. Задай пользователю три вопроса **одним сообщением**:
...
1. Запиши выбор в `.env` как комментарии:
   ```
